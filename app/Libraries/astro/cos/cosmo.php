...
Function CalculatePositions($last_id, $swephsrc="",$sweph="")
  ...
  // $PATH="C:/xampp/htdocs/q10/cos";
    ...
    $my_latitude = $row['ns'] * ($row['lat_deg'] + ($row['lat_min'] / 60));
      ...
      $utdatenow = DateTimeImmutable::createFromFormat
      ('U', mktime($inhours, $inmins, $insecs, $inmonth, $inday, $inyear));
      $utdaten=$utdatenow->format('d:m:Y');

      $utnow =DateTimeImmutable::createFromFormat
      ('U', mktime($inhours, $inmins, $insecs, $inmonth, $inday, $inyear));
    ...
    $sw=exec("$sweph/swetest -edir$sweph -b$utdaten -ut$utnw -p0123456789 -eswe -house$my_longitude,$my_latitude, -fPldj -g, -head 2>&1 ", $out);
     
    //https://www.php.net/manual/en/function.exec.php
    // $struenode=exec("$sweph/swetest -edir$sweph -b$utdaten -ut$utnw 
    // -p0123456789Dmte -eswe -house$my_longitude,$my_latitude, -fPjZ  -roundsec  -g, -head 2>&1", $outrue);    
    //   // $truenode= ;
    // $struenode2=exec("$sweph/swetest -edir$sweph -b$utdaten -ut$utnw -g -fTlbR
    // -p0123456789Dte -eswe -house$my_longitude,$my_latitude, -hor -n366 -roundsec  -g, 2>&1 ", $outrue);    
    // $struenode3=exec("$sweph/swetest -edir$sweph -b$utdaten -ut$utnw -g -fTlbR
    // -p0123456789Dte -eswe -house$my_longitude,$my_latitude, -g -fTlbR -p0123456789Dmte -hor -n366 -roundsec", $outrue);    
    
    // $struenode4=exec("$sweph/swetest -edir$sweph -b$utdaten -ut$utnw -g -fTlbR
    // -p0123456789Dte -eswe -house$my_longitude,$my_latitude, -g -fTlbR -p0123456789Dmte -hor  -roundsec", $outrue);    
    // $struenode5=exec("$sweph/swetest -edir$sweph -b$utdaten -ut$utnw -g -fTlbR
    // -p0123456789Dte -eswe -house$my_longitude,$my_latitude, -g -fPjZ -p0123456789Dmte -hor  -roundsec", $outrue);  

    $struenode=exec("$sweph/swetest -edir$sweph -b$utdaten -ut$utnw -p0123456789Dmt -eswe -house$my_longitude,$my_latitude,   -fPldj -g,  -head  -roundsec 2>&1", $outrue); 

    // $struenode2=exec("$sweph/swetest -edir$sweph -b$utdaten -ut$utnw -p0123456789Dmt -eswe -house$my_longitude,$my_latitude, -g, -hor  -roundsec", $outrue2);   
  ...
  /*echo("</br>");
    ...
    //truenode appears in 12th position of outrue , the rest is shifted by 2 
    // foreach ($outrue as $key => $line)
    //   {
    //     $row = explode(',',$line);
    //     $pl_name[$key] = $row[0];
    //     print_r($row[0]);echo("<span>   </span>");
    //     $longitude[$key] = $row[1];
    //     print_r($row[1]);echo("<span>   </span>");
    //     $declination[$key] = $row[2];
    //     print_r($row[2]);echo("<span>   </span>");
    //     $house_pos[$key] = $row[3];
    //     print_r($row[3]);
    //     echo("</br>");
    //   };
